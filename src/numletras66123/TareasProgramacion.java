...
class Num_a_letras {
    public static String cantidadConLetra(String s) {
        ...
        if (parteEntera == 0) { result.append("Cero "); return result.toString();
        }
        if (Miles > 0)       result.append(triTexto(Miles).toString() + "Mil ");
        if (Unidades > 0)    result.append(triTexto(Unidades).toString());
    ...
    private static StringBuilder triTexto(int n) {
        ...
        switch (centenas) {
            case 0: break;
            case 1:
                if (decenas == 0 && unidades == 0) {result.append("Cien ");
                    return result;
                }
